...
const ImportPage: React.FC = () => {
  ...
  const bookmarklet =
    `javascript:(async()=>{const r=await fetch('https://new.chunithm-net.com/chuni-mobile/html/mobile/collection/characterList/',{credentials:'include'}),d=new DOMParser().parseFromString(await r.text(),'text/html'),baseUrl='https://new.chunithm-net.com/chuni-mobile/html/mobile/img/',chars=[...d.querySelectorAll('.character_list_block')].map(e=>{const n=e.querySelector('.character_name_block a')?.textContent.trim()||'',charaInput=e.querySelector('input[name="chara"]'),id=charaInput?'ipId14-'+charaInput.value:'';if(!id)return;const img=e.querySelector('.list_chara_img img')?.getAttribute('data-original')||'no_image.png',rk=[...e.querySelectorAll('.character_list_rank_num img')].map(x=>{const src=x.getAttribute('src')||'';const match=src.match(/num_s_lv_(\\d)\\.png/);return match?match[1]:''}).filter(x=>x).join(''),mx=!!e.querySelector('.character_list_rank_max');return{name:n,charaId:id,imgSrc:img,rank:rk,isMax:mx}}).filter(Boolean),compressed=chars.map(c=>[c.name,c.charaId,c.imgSrc.startsWith(baseUrl)?c.imgSrc.substring(baseUrl.length):c.imgSrc,c.rank,c.isMax?1:0]),compressedData=btoa(encodeURIComponent(JSON.stringify(compressed))),url='https://Suu0313.github.io/character-rank-manager-net/import?data='+compressedData;if(compressedData.length>20000){const t=document.createElement('textarea');t.value=JSON.stringify(chars,null,2);t.style='position:fixed;top:10px;left:10px;width:90vw;height:50vh;z-index:9999;';document.body.appendChild(t);setTimeout(()=>{t.select();t.setSelectionRange(0,t.value.length);},0);alert('データが大きすぎるため、手動でコピーしてください。');}else{window.open(url,'_blank');}})();`;
